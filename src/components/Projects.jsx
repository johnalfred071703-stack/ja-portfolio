...
const projects = [
  ...
  },
  {
    ...
    gallery: ['/mwms-figma.png', '/mwms-testcase.png', '/mwms-accounts.png'],
  ...
  },
  {
    ...
    gallery: ['/zentive-4.png', '/zentive-1.png', '/zentive-2.png', '/zentive-3.png'],
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        onClick={onClose}
        style={{ position: 'fixed', inset: 0, background: overlayBg, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
        <motion.div
          initial={{ opacity: 0, y: 40, scale: 0.96 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 40 }}
          onClick={e => e.stopPropagation()}
          style={{ background: bg, borderRadius: '20px', border: `1px solid ${border}`, maxWidth: '780px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative', boxShadow: '0 32px 80px #00000040', scrollbarWidth: 'thin', scrollbarColor: `#00e676 #1a1a1a` }}>
          <button onClick={onClose}
            style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'transparent', border: `1px solid ${border}`, borderRadius: '50%', width: '36px', height: '36px', cursor: 'pointer', color: textDim, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
            <FiX />
          </button>
...
function Projects({ onModalChange }) {
  const { dark } = useTheme();
  ...
  const handleOpen  = (p) => { setSelected(p); onModalChange(true); };
  const handleClose = () => { setSelected(null); onModalChange(false); };
  ...
  return (
    <section id="projects" style={{ minHeight: '100vh', padding: '5rem 3rem', position: 'relative', zIndex: 1, background: bg, transition: 'background 0.3s' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', pointerEvents: selected ? 'none' : 'auto' }}>
      ...
      {selected && <ProjectModal project={selected} onClose={handleClose} dark={dark} />}
    </section>
  );
}
