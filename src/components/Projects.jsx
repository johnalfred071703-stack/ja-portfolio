...
const projects = [
  ...
  },
  {
   ...
   tags: ['React', 'JavaScript', 'HTML/CSS', 'Bug Fixing', 'UI Development'],
    ...
    gallery: ['/zentive-1.png', '/zentive-2.png', '/zentive-3.png'],
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        onClick={onClose}
        style={{ position: 'fixed', inset: 0, background: overlayBg, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <motion.div
          initial={{ opacity: 0, y: 40, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 40 }}
          onClick={e => e.stopPropagation()}
          style={{ background: bg, borderRadius: '20px', border: `1px solid ${border}`, maxWidth: '780px', width: '100%', maxHeight: '88vh', overflowY: 'auto', position: 'relative', boxShadow: '0 32px 80px #00000040' }}>

          {/* Close */}
          <button onClick={onClose} style={{ position: 'absolute', top: '1.2rem', right: '1.2rem', background: 'transparent', border: `1px solid ${border}`, borderRadius: '50%', width: '36px', height: '36px', cursor: 'pointer', color: textDim, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
            <FiX />
          </button>

          {/* Main image */}
          <div style={{ borderRadius: '20px 20px 0 0', overflow: 'hidden', height: '320px', background: '#0d0d0d' }}>
            <img src={project.gallery[activeImg]} alt={project.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>

          {/* Thumbnails */}
          <div style={{ display: 'flex', gap: '0.6rem', padding: '1rem 1.5rem 0' }}>
            {project.gallery.map((img, i) => (
              <div key={i} onClick={() => setActiveImg(i)}
                style={{ width: '72px', height: '48px', borderRadius: '8px', overflow: 'hidden', border: `2px solid ${i === activeImg ? accent : border}`, cursor: 'pointer', opacity: i === activeImg ? 1 : 0.6, transition: 'all 0.2s' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
        {projects.map((p, i) => (
          <motion.div key={i}
            ...
            <div style={{ height: '200px', overflow: 'hidden', background: '#0d0d0d', position: 'relative' }}>
              <img src={p.gallery[0]} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.9 }} />
            ...
            <div style={{ padding: '1.4rem' }}>
              <h3 style={{ fontSize: '1.15rem', fontWeight: 700, color: textMain, marginBottom: '0.6rem' }}>{p.title}</h3>
